...
import rs.ac.metropolitan.mushroomiden.data.remote.dto.toIdentificationResult
...
class GetIdentificationUseCase @Inject constructor(private val repository: MushroomIdentificationRepository){

    operator fun invoke(identificationRequest: IdentificationRequest): Flow<Resource<IdentificationResult>> = flow {
        try {
            emit(Resource.Loading<IdentificationResult>())
            val result = repository.getIdentification(identificationRequest).toIdentificationResult()
